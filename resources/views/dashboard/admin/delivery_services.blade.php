            ...
            <div class="card-body">
                <div class="">
                    <div class="text-center">
                        ...
                        data-target="#add_faq_category">Add Delivery Service</button>
                    </div>
                    <div class="table-responsive">
                        @isset($delivery_services)
                            <table id="example" class="table table-bordered text-nowrap key-buttons">
                                ...
                                <tbody>
                                @foreach($delivery_services as $delivery)
                                    <tr>
                                        <td>{{$delivery->name}}</td>
                                        <td>{{$delivery->phone}}</td>
                                        <td>
                                            @if ($delivery->logoUploaded ==1)
                                                <img alt="Logo" class="avatar avatar-md brround"
                                                src="{{asset('merchant/photos/'.$delivery->logo)}}"
                                                style="width:80px;height:80px;">
                                            @else
                                                <img alt="Logo" class="avatar avatar-md brround"
                                                src="http://ui-avatars.com/api/?name={{$delivery->name}}&rounded=true&background=random"
                                                style="width:80px;height:80px;">
                                            @endif
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <a href="{{url('admin/delivery_services/edit/'.$delivery->id)}}" class="btn btn-outline-info">
                                                <i class="fa fa-edit"></i>
                                            </a>
                                            <a href="{{url('admin/delivery_location/'.$delivery->id)}}" class="btn btn-outline-info">
                                                <i class="fa fa-eye"></i>
                                            </a>
                                            <a href="{{url('admin/delivery_services/delete/'.$delivery->id)}}" class="btn btn-outline-danger">
    ...
    <div class="modal-dialog modal-dialog-centered text-center " role="document">
        <div class="modal-content">
            ...
            <div class="modal-body text-left p-4" >
                <form method="POST" action="{{url('admin/delivery_services/add')}}" enctype="multipart/form-data">
                    @csrf
                    <div class="row">
