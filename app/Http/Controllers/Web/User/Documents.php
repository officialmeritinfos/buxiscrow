...
class Documents extends  BaseController
{
    public function index(){
        ...
        return view('dashboard.user.verify_account', $dataView);
    }
    public function verifyAccount(Request $request){
        ...
        $move1 = $request->file('front_image')->move(public_path('user/photos/'), $fileName1);
        //move the second image
        if($docType->hasBack ==1){
            $fileName2 = time() . '_' . $request->file('back_image')->getClientOriginalName();
            $move2 = $request->file('back_image')->move(public_path('user/photos/'), $fileName2);
        }else{
            $fileName2='';
            $move2='';
        }
