...
use App\Models\Payouts as transfers;
...
class Payouts extends BaseController
{
    ...
    public function index(){
        ...
        $user = Auth::user();
        $transfers = transfers::where('user',$user->id)->get();
        ...
        return view('dashboard.user.payouts', $dataView);
    }
    ...
    public function authenticateNGNTransfer($data,$user,$ben){
        ...
        $userBalance = UserBalances::where('user',$user->id)->where('currency',strtoupper($data->input('currency')))->first();
        ...
        $dataTransfer = [
            ...
            'reference'=>$reference,
            'narration'=>$narration,
            'callback_url'=>config('app.url').'/withdrawal_callback/user/'.$user->id.'/reference/'.$reference,
        ];
        ...
        if ($transfer->ok()){
            ...
            $dataCharge = [
                'amount'=>$data->input('amount'),'charge'=>$charge,'flutCharge'=>$transfer['data']['fee'],'currency'=>$data->input('currency'),
                'reference'=>$reference
            ];
            //create records
            ...
            if (!empty($addTransactions)){
                ...
                $dataActivity = ['user' => $user->id, 'activity' => 'Account Transfer', 'details' => $details,
                    'agent_ip' => $data->ip()];
    ...
    public function payoutDetails($ref){
        $generalSettings = GeneralSettings::where('id',1)->first();
        $user=Auth::user();
        $transfer = transfers::where('user',$user->id)->where('reference',$ref)->first();
        ...
        return view('dashboard.user.payout_details',$dataView);
    }
}
