...
class Settings extends BaseController
{
    ...
    public function index()
    {
        ...
        return view('dashboard.merchant.settings', $dataView);
    }
    ...
    public function updateProfilePic(Request $request)
    {
        ...
        if ($move) {
            ...
            if (!empty($updated)) {
                ...
                $dataActivity = ['merchant' => $user->id, 'activity' => 'Photo update', 'details' => $details, 'agent_ip' => $request->ip()];
    ...
    public function updatePassword(Request $request)
    {
        $user = Auth::user();
        $validator = Validator::make($request->all(),
            ['pin' => ['bail', 'required', 'digits:6'], 'old_password' => ['bail', 'required'], 'new_password' =>
                [Password::min(8)->letters()->mixedCase()->numbers()->symbols(), 'required'],
        ...
        if ($hashed_pin && $hashed) {
            ...
            if (!empty($updated)) {
                ...
                $dataActivity = ['merchant' => $user->id, 'activity' => 'Password reset', 'details' => $details, 'agent_ip' => $request->ip()];
                event(new AccountActivity($user, $dataActivity));
                $success['reset'] = true;
        ...
        return $this->sendError('Password Error ', ['error' => 'Wrong Credentials. Crosscheck both your old password and account pin'], '422', 'password change Fail');
    }
        ...
        if (!empty($stateExistsInCountry)){
            ...
            if (!empty($cityExistsInCountry)){
                ...
                if (!empty($update)){
                    ...
                    $dataActivity = ['merchant' => $user->id, 'activity' => 'Profile Update', 'details' => $details, 'agent_ip' => $request->ip()];
                    event(new AccountActivity($user, $dataActivity));
                    $success['reset'] = true;
    ...
    public function updateSecurity(Request $request)
    {
        ...
        if ($hashed_pin) {
            ...
            if (!empty($updated)) {
                ...
                $dataActivity = ['merchant' => $user->id, 'activity' => 'Security update', 'details' => $details, 'agent_ip' => $request->ip()];
                event(new AccountActivity($user, $dataActivity));
                $success['update'] = true;
