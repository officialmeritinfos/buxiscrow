...
use App\Models\MerchantBeneficiary;
...
class Beneficiary extends BaseController
{
    public function index(){
        ...
        return view('dashboard.merchant.beneficiaries',$dataView);
    }
        ...
        if (empty($beneficiaryExists)){
            ...
            if ($createBaneficiary->ok()){
                ...
                if (!empty($addBeneficiary)){
                    ...
                    $dataActivity = ['merchant' => $user->id, 'activity' => 'Transfer Receipient', 'details' => $details, 'agent_ip' => $request->ip()];
    ...
    public function removeBeneficiary(Request $request,$beneficiary_id){
        ...
        if (!empty($beneficiaryExistsAnother)){
            ...
            $dataActivity = ['merchant' => $user->id,'activity' => 'Transfer Recipient Removal', 'details' => $details, 'agent_ip' => $request->ip()];
            event(new AccountActivity($user, $dataActivity));
