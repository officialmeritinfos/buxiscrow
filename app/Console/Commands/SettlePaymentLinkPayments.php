...
class SettlePaymentLinkPayments extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'settle:paymentLinkPayments';
    ...
    {
        $moment = time();
        $payments = PaymentLinkPayments::where('paymentStatus',1)->where('settled','!=',1)->where('timeToSettle','<=',$moment)->get();
        if ($payments->count()>0){
            foreach ($payments as $payment) {
                ...
                $update = PaymentLinkPayments::where('id', $payment->id)->update($dataPayment);
                if ($update) {
                    MerchantBalances::where('id', $userBalance->id)->update($dataBalance);
                    //mail to user
                    $mail2 = 'Your pending settlement of <b>' . $payment->currency . number_format($payment->amountCredit, 2) . '</b> on
